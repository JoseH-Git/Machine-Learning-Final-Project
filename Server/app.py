...
def jsonified():
    ...
    user = [i for i in collection.find(filter_data())]    
    for user in users: user.pop("_id")

    # This is commented only for testing
    # response = jsonify(songs)
    # response.headers.add('Access-Control-Allow-Origin', '*')
    # return response
    response = group_data(user, group,"User",[group,"User"]) 
...
def jsonified4():
    ...
    users = [i for i in mongo.db.Empployees.find()]    
    for user in users: User.pop("_id")
